  ...
  const handleMarkAsWatched = async () => {
    try {
      const response = await fetch("/api/watched", {
        ...
        body: JSON.stringify({ fromWatchlistId: watchlistItem.id }),
      });
    ...
    <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg">
      <div className="relative aspect-[2/3] bg-muted">
        ...
        <div className="absolute top-2 right-2">
          <Badge variant="secondary" className="flex items-center gap-1">
            ...
            {watchlistItem.vote_average?.toFixed(1)}
          </Badge>
        </div>
      </div>
      ...
      <CardContent className="p-4">
        ...
        <p className="text-sm text-muted-foreground mt-1 line-clamp-2">
          {watchlistItem.overview.substring(0, 60)}
          {watchlistItem.overview.length > 60 ? "..." : ""}
        </p>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          <Badge variant="outline">
            {watchlistItem.media_type === "movie" ? "Movie" : "TV Show"}
          </Badge>
        </div>
      </CardContent>

      <CardFooter className="p-4 flex gap-2">
        <Button size="sm" className="flex-1" onClick={handleMarkAsWatched}>
