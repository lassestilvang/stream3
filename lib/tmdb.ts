...
export const getMovieDetails = async (id: number, mediaType: 'movie' | 'tv'): Promise<Movie> => {
  if (!TMDB_API_KEY) {
    throw new Error('TMDB API key is not configured');
  }
...
export const getImageUrl = (path: string | undefined, size: 'w500' | 'w780' | 'w1280' | 'original' = 'w500'): string => {
  if (!path) return '';
