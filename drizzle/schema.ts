...
export const verificationTokens = pgTable(
  ...
  (vt) => ({
    compoundKey: vt.compoundKey({ columns: [vt.identifier, vt.token] }),
  })
);

// Watched content table
export const watchedContent = pgTable('watched_content', {
  id: varchar('id', { length: 255 }).primaryKey().notNull().defaultRandom(),
  userId: varchar('userId', { length: 255 })
    .notNull()
    .references(() => users.id, { onDelete: 'cascade' }),
  ...
  mediaType: varchar('media_type', { length: 10 }).$type<'movie' | 'tv'>().notNull(),
